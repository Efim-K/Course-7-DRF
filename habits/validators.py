    ...
    def __call__(self, value):
        ...
        if val.get("award") and val.get("related_habit"):
            raise ValidationError(
                "Поле вознаграждения и связанная привычка не могут быть заполнены одновременно"
            )

        # Время выполнения должно быть не больше 120 секунд
        if val.get("complete_time") and val["complete_time"] > timedelta(seconds=120):
            raise ValidationError(
        ...
        if val.get("related_habit"):
            if not Habit.objects.filter(
                pk=val["related_habit"], is_pleasant=True
            ).exists():
